...
for(i in 1:length(split_list)){
  split_list[[i]][ , 2:ncol(split_list[[i]])] <- apply(split_list[[i]][ , 2:ncol(split_list[[i]])], 2, function(x) (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE))
}
...
depvar_list <- lapply(depvar_list, function(x) x[match(poly_nuts, x$NUTS_ID), ])

for(i in 1:length(depvar_list)){
  
  depvar_list[[i]][ , 2:ncol(depvar_list[[i]])] <- apply(depvar_list[[i]][ , 2:ncol(depvar_list[[i]])], 2, log)
